...
until SHIP:VERTICALSPEED >= -0.01 and SHIP:VELOCITY:SURFACE:MAG < 1 {
  ...
  if accel > g {
    ...
    set desired_vy to max(touchdownv, SQRT(2*max(0, p_altitude - 10)*(max(0, accel*sin(theta_optimal) - g)))). // the maximum vertical velocity that can be cancelled out at current altitude and thrust capacity at optimal angle
