...
until SHIP:VERTICALSPEED >= -0.01 and SHIP:VELOCITY:SURFACE:MAG < 1 {
  ...
  if accel > g {
    ...
    set desired_alt to SHIP:VERTICALSPEED*SHIP:VERTICALSPEED/(2*accel*SIN(theta)). // the maximum altitude that can be cancelled out at max thrust with this landing pattern
    
    set throt to desired_alt - ALT:RADAR.
    lock throttle to min(max(throt, 0), 1).
